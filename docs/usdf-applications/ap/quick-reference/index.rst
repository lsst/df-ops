...
.. The following diagram mirrors the reference architecture diagram. It shows the complete data flow from Summit to community alert brokers.

.. .. mermaid::

..    flowchart TB
..       subgraph Summit["☁️ Summit"]
..          SQ["ScriptQueue"]
..          SSK[("Summit Sasquatch<br/>Kafka")]
..          SQ -- "nextVisit" --> SSK
..       end

..       subgraph USDF["USDF — usdf-prompt-processing vCluster"]

..          subgraph NetLayer["Network Layer"]
..             SOCAT["socat proxy<br/><i>kafka-proxy ns</i>"]
..          end

..          subgraph Orchestration["Orchestration"]
..             NVFO["Fanout Service<br/>(Next Visit Fan Out)<br/><i>next-visit-fan-out ns</i>"]
..             REDIS[("Prompt Redis<br/>(Redis Streams)<br/><i>prompt-redis ns</i>")]
..             KEDABOX["KEDA Autoscaler<br/><i>keda ns</i>"]
..          end

..          subgraph PodGroup["Scaled Job Pods  (× N detectors)"]
..             subgraph PodInternals["Pod Internals — Payload"]
..                SJ["Scaled Job<br/>Consumer"]
..                PRELOAD["Preload Phase"]
..                APP["Alert Production<br/>Pipeline"]
..                LOCALREPO[("Local Repo<br/>(pod-local Butler)")]
..                EXPORT["Export Outputs"]
..             end
..          end

..          subgraph ButlerWriter["Data Persistence"]
..             PK[("Prompt Kafka<br/>(Strimzi)<br/><i>prompt-kafka ns</i>")]
..             BW["Butler Writer<br/>Service<br/><i>butler-writer-service ns</i>"]
..             PPS["Prompt Publication<br/>Service"]
..          end
..       end

..       subgraph ExtSystems["External Systems & Storage"]
..          S3NK[("S3 File Notifications<br/>Kafka")]
..          CR[("Central Repo<br/>(Embargo Butler<br/>read-only replica)")]
..          APDB[("APDB<br/>Cassandra<br/>sdfk8sk001-006")]
..          SASQ[("Sasquatch<br/>(USDF RSP)")]
..          ASB["Alert Stream Broker<br/>→ Alert Kafka<br/>→ Community Brokers"]
..          SATTLE["Sattle<br/>(Embargo Rack)"]
..          PRESENCE["Presence<br/>Microservice<br/><i>usdf-embargo-dmz</i>"]
..       end

..       SSK -- "LHN" --> SOCAT
..       SOCAT --> NVFO
..       NVFO -- "fan out per<br/>active detector" --> REDIS
..       REDIS -. "pending entries<br/>count trigger" .-> KEDABOX
..       KEDABOX -- "autoscale<br/>up/down" --> SJ

..       SJ -- "nextVisit event" --> PRELOAD
..       S3NK -- "objectCreated" --> SJ
..       PRESENCE -- "image URL<br/>lookup" --> PRELOAD
..       PRELOAD -- "read calibrations<br/>& templates" --> CR
..       PRELOAD -- "read previous<br/>detections" --> APDB
..       PRELOAD --> APP

..       APP -- "write new<br/>DIASources" --> APDB
..       APP -- "publish metrics<br/>via REST Proxy" --> SASQ
..       APP -- "filter via<br/>satellite catalog" --> SATTLE
..       APP --> LOCALREPO
..       LOCALREPO --> EXPORT

..       EXPORT -- "dataset records" --> PK
..       EXPORT -- "alerts" --> ASB
..       PK --> BW
..       PK --> PPS
..       BW -- "batch insert" --> CR

..       style Summit fill:#fff3e0,stroke:#e65100,stroke-width:2px
..       style USDF fill:#e3f2fd,stroke:#1565c0,stroke-width:2px
..       style ExtSystems fill:#fce4ec,stroke:#c62828,stroke-width:2px
..       style PodGroup fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px
