...
images + 8 guider images + 8 wavefront images) every minute to the USDF embargo CEPH S3 storage.
All images from an exposure (and the associated .json files) are saved to a foler like
...
- via a Webhook to the ``embargo-butler-enqueue`` (https://172.24.5.156:8080/notify) service running
  ...
  ``summit-new`` namespace will then ingest the new raw images to the ``embargo`` Butler).
- vai a Kafka message to trigger Prompt processing.

(not sure how RA and NV are triggered).
...
operation: If the notification sending fails, the object not be saved.
...
Detect Exposures Fail to Unembargo
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
out which exposures are not fully unembargoed.
